...
use App\Models\asignarAutorizacionModelo;


class asignarAutorizacionControlador extends Controller
{
    //

    public function index()
    {
        $autorizacion = asignarAutorizacionModelo::all();

        // Verifica si no hay registros
        if ($autorizacion->isEmpty()) {
            $data = [
                'message' => 'No hay  usuarios ni autorizaciones registradas',
                'status' => 404
            ];
    ...
    public function store(Request $request)
    {
        ...
        $autorizacion = asignarAutorizacionModelo::create([
            'id' => $request->id,
        ...
        if (!$autorizacion) {
            $data = [
                'message' => 'Error al asignar la autorizacion',
                'status' => 500
            ];
        ...
        $data = [
            'message' => 'Autorizacion asignada correctamente',
    ...
    $autorizacion = asignarAutorizacionModelo::find($id);
    if (!$autorizacion) {
        $data = [
            'message' => 'usuario o autrorizacion no encontrada',
...
public function destroy($id){
    ...
    if (!$autorizacion) {
        $data = [
            'message' => 'Ausuario o autrorizacion no encontrada',
            'status' => 404 // verifica si la ruta existe
        ];
...
public function update(Request $request, $id){
    ...
    $data = [
        'message' => 'usuario y autorizacion actualizados correctamente',
        'autorizacion' => $autorizacion,
        'status' => 200 // solicitud exitosa
    ];
